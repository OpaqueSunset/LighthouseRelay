...
/datum/weather/virgo3b/snow/process_effects()
	...
	for(var/turf/floor/outdoors/snow/S in SSplanets.new_outdoor_turfs) //This didn't make any sense before SSplanets, either
		if(S.z in holder.our_planet.expected_z_levels)
			for(var/dir_checked in cardinal)
				var/turf/floor/T = get_step(S, dir_checked)
				if(istype(T))
					if(istype(T, /turf/floor/outdoors) && prob(33))
						T.chill()
	...
	for(var/turf/floor/outdoors/snow/S in SSplanets.new_outdoor_turfs) //This didn't make any sense before SSplanets, either
		if(S.z in holder.our_planet.expected_z_levels)
			for(var/dir_checked in cardinal)
				var/turf/floor/T = get_step(S, dir_checked)
				if(istype(T))
					if(istype(T, /turf/floor/outdoors) && prob(50))
						T.chill()
